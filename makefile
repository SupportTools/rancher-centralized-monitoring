...
# vars
IMAGENAME=rancher-monitoring-relay
...
security:
	...
	@gosec -fmt sarif -out gosec-results.sarif ./... || echo "Gosec found issues - check gosec-results.sarif"
	@echo "2. Running Trivy filesystem scan..."
	@docker run --rm -v $$(pwd):/src aquasec/trivy:latest fs --severity HIGH,CRITICAL --format sarif --output /src/trivy-fs-results.sarif /src || echo "Trivy filesystem scan found issues"
	@echo "3. Running GitLeaks..."
	@docker run --rm -v $$(pwd):/src gitleaks/gitleaks:latest detect --source /src --report-format sarif --report-path /src/gitleaks-results.sarif || echo "GitLeaks found secrets"
	@echo "Security scans completed! Check *.sarif files for results"
